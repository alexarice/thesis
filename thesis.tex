...
\begin{corollary}
  ...
  \begin{itemize}
  \item There exists some \(B: \Type_\Gamma\) such that \(\Gamma \vdash \Coh {D^n} {U^n} {\{A,t\}} : B\),
  \item \(\Gamma \vdash \Coh{D^n} {U^n} {\{A,t\}} : A\).
  \end{itemize}
  where \(n = \dim(A)\).
\end{corollary}
\begin{proof}
...
\begin{proof}
  ...
  \[ ((\Gamma, (x : B)), \Coh {D^n} {U^n} {\wk(\{A,t\})}, \wk(t)) \in \dr \]
...
\end{proposition}
\begin{proof}
  The proof is similar to \cref{prop:dr-weak,prop:dr-susp}, except for this proof the equality \(\{A,t\} \bullet \sigma \equiv \{A \sub \sigma, t \sub \sigma\}\) from \cref{item:disc-prop-sub-sub} is crucial.
\end{proof}

\begin{remark}
  The proof of the substitution condition for \dr makes no use of the typing of \(\sigma\). In fact this premise is only necessary for the supported rules construction which will be given in
\end{remark}
\todo[inline]{reference above}
...
\[ \{(D^n, \Coh {D^n} {U^n} {\id_{D^n}}, d_n) \mid n \in \mathbb{N}\} \]
...
\begin{lemma}
  ...
  \item \(\Supp(\sigma) = \FV(\sigma)\) when \(\Gamma \vdash \sigma : \Delta\),
  \item \(\Supp(t) = \Supp(A) \cup \FV(t)\) when \(\Gamma \vdash t : A\),
  \item \(\Supp(t) = \FV(A) \cup \FV(t) = \Supp(A) \cup \Supp(t)\) when \(\Gamma \vdash t : A\) and \(\Gamma \vdash A\).
  \end{lemmaenum}
\end{lemma}
\begin{proof}
