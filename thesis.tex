...
The definition of \(\infty\)-categories given in the previous section is close in spirit to the the ordinary definitions of 1-categories and clearly demonstrates the different families of axioms present. However, we will see in \cref{sec:strictness}, that this sort of definitions do not scale well to our eventual setting of weak higher categories.

The elements of the free strict \(\infty\)-category on a globular set \(G\) can instead be represented by the pairing of a special class of globular set known as a \emph{pasting diagram}, and a map from this pasting diagram into \(G\). Due to this equivalence, the compositional structure of an infinity category is completely described by these pasting diagrams. From any pasting diagram we must be able to obtain an element of the free \(\infty\)-category, and this element must be unique.

Informally, we can define a pasting diagram to be a finite globular set which admits a unique full composite, where a full composite of a globular set \(G\) is an element of the free \(\infty\)-category over \(G\) which uses all the maximal elements. This functions as the primary intuition on the role of pasting diagrams, but other more combinatorial definitions are easier to use and manipulate in practice.

Before giving a more formal definition of pasting diagrams, we explore some examples and non-examples.
...
\begin{definition}
  ...
  The boundary of an \((n+1)\)-dimensional pasting diagram is given by replacing each instance of \(D^{n+1}\) by \(D^n\) in it's globular sum representation. There are two canonical maps including the boundary into the original pasting diagram, whose images give the source and target of the pasting diagram.
\end{definition}
...
\begin{example}
  ...
  \[
    \begin{tikzcd}
      ...
      \arrow["\alpha", shorten <=5pt, shorten >=5pt, Rightarrow, nfold, from=1, to=0]
      \arrow["\beta", shorten <=3pt, shorten >=3pt, Rightarrow, nfold, from=3, to=4]
      \arrow["\gamma", shorten <=3pt, shorten >=3pt, Rightarrow, nfold, from=4, to=2]
    \end{tikzcd}
  \]
  ...
  \[
    \begin{tikzcd}[column sep=tiny, row sep = small]
      ...
      \arrow["\alpha", shorten <=5pt, shorten >=5pt, Rightarrow, nfold, from=1, to=0]
      \arrow["\beta", shorten <=3pt, shorten >=3pt, Rightarrow, nfold, from=2, to=3]
      \arrow["\gamma", shorten <=3pt, shorten >=3pt, Rightarrow, nfold, from=5, to=6]
