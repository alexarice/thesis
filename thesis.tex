...
For every \(n\)-cell \(x\), there is an \((n+1)\)-cell \(\iden{x} : x \to x\), called the \emph{identity morphism}.
...
\[ \iden{\iden{x}} \star_0 \alpha = \alpha \]
...
\begin{definition}
  ...
  \end{align*}
  for all \(n \in \mathbb{N}\). A morphism of globular sets \(F : \mathcal{G} \to \mathcal{H}\) is a collection of functions \(f_n : G_n \to H_n\) which commute with source and target maps.
\end{definition}

Given a globular set \(\mathcal{G}\), we will call the elements of \(G_n\) the \(n\)-cells and write \(f : x \to y\) for an \((n+1)\)-cell \(f\) where \(s_n(f) = x\) and \(t_n(f) = y\). Further we will drop the subscripts on \(s_n\) and \(t_n\) when they are clear. Further, we write \(s^n\) and \(t^n\) to denote \(s\) or \(t\) applied \(n\) times. We can now give the definition of a (strict) \(\infty\)-category.

\todo[inline]{Add coinductive definition?}

\begin{definition}
  A (strict) \(\infty\)-category is a globular set \(\mathcal{G}\) with operations:
  \begin{itemize}
  \item For \(m < n\), a composition \(\star_m\) taking \(n\)-cells \(f\) and \(g\) with \(t^{n-m}(f) = s^{n-m}(g)\) and giving an \(n\)-cell \(f \star_m g\) with:
    \begin{align*}
      ...
      \end{cases*}
    \end{align*}
  \item For \(n\)-cell \(x\), an identity \((n+1)\)-cell \(\iden{x} : x \to x\).
  \end{itemize}
  ...
  \item Unitality: Given \(m < n\) and \(n\)-cell \(f\):
    \begin{align*}
      \iden{s^{n-m}(f)} \star_m f &= f\\
      f \star_m \iden{t^{n-m}(f)} &= f
    \end{align*}
  \item Composition interchange: If \(m < m' < n\) and \(\alpha\), \(\beta\), \(\gamma\), and \(\delta\) be \(n\)-cells with
    \[t^{n-m'}(\alpha) = s^{n-m'}(\beta)\qquad
      t^{n-m'}(\gamma) = s^{n-m'}(\delta)\qquad
      t^{n-m}(\alpha) = s^{n-m}(\gamma)\]
    then:
    \[(\alpha \star_m \gamma) \star_{m'} (\beta \star_m \delta) = (\alpha \star_{m'} \beta) \star_m (\gamma \star_{m'} \delta)\]
  \item Identity interchange: Let \(m < n\) and \(f\) and \(g\) be \(n\)-cells with \(t^{n-m}(f) = s^{n-m}(g)\). Then:
    \[\iden{f} \star_m \iden{g} = \iden{f \star_m g}\]
  \end{itemize}
  A morphism of \(\infty\) categories is a morphism of the underlying globular sets which preserves composition and identities.
\end{definition}

\todo[inline]{Free infty cat, monad}
