...
We now give our first example of an equality rule, \emph{disc removal}. Disc removal removes unary composites, replacing them with the underlying term. We recall from \cref{sec:basic-constructions} that for every \(n\), there exists the \(n\)-dimensional disc context \(D^n\), and that given a term \(t \in \Term_\Gamma\) and \(n\)-dimensional type \(A \in \Type_\Gamma\), there exists a substitution \(\{A,t\} : D^n \to \Gamma\). The unary composite of a term \(t\) of type \(A\) of dimension \(n > 0\) is then the coherence:
\[\Coh {D^n} {d_{n-1}^- \to d_{n-1}^+} {\{A,t\}}\]
Disc removal equates this with the term \(t\), making the following rule inferable:
\begin{mathpar}
  \inferrule{\Gamma \vdash t : A}{\Gamma \vdash \Coh {D^n} {d_{n-1}^- \to d_{n-1}^+} {\{A,t\}} = t}\textsc{dr}
\end{mathpar}
...
\begin{definition}
  ...
  \[ (\Gamma, \Coh {D^{n+1}} {d_n^- \to d_n^+} {\sigma}, d_{n +1}\sub\sigma) \]
  for each \(n \in \mathbb{N}\), context \(\Gamma\), and substitution \(\sigma : \arr {D^{n+1}} \star \Gamma\).
\end{definition}

We note that the set \dr contains the unary composites written above as for an \(n\)-dimensional type \(A\), we have \(d_n\sub{\{A,t\}} \equiv t\). The inference rule \textsc{dr} follows the \textsc{rule} and typing properties about discs given in \cref{sec:ruleset}.

We draw attention to the typing premise of \textsc{rule}. If we know that the unary composite of a term \(t\) is well typed, then it follows that \(t\) itself must have been well-typed, but we cannot infer that the term \(\Coh {D^n} {d_{n-1}^- \to d_{n-1}^+} \sigma\) is well-typed from \(d_n \sub \sigma\) being well-typed. In particular, the well-formedness of \(d_n \sub \sigma\) does not constrain the lower dimensional terms of \(\sigma\) at all. We must therefore including the typing premise if we want to avoid well-typed and untyped terms being equated.
...
% allow space for figure
\clearpage
\section{The set of operations \texorpdfstring{\(\mathcal{O}\)}{O}}
\label{sec:support}

\todo[inline]{Main purpose is to manage groupoidal things}

\subsection{Operation sets}
\label{sec:operation-sets}

\todo[inline]{Explicitly give boundary sets}

\todo[inline]{List of bools in formalisation}

\todo[inline]{Boundary sets are downwards closed}

\todo[inline]{Definition of operation set}

\todo[inline]{Remark about proof relevance}

\todo[inline]{Introduce regular operations}

\todo[inline]{Introduce groupoidal Catt}

\todo[inline]{Standard operation sets}
\todo[inline]{Equivalence}

\todo[inline]{Remark about globular catt}

\todo[inline]{Mention things this does not contain}

\subsection{Operation properties}
\label{sec:operation-properties}

\todo[inline]{Explain need for some properties}

\paragraph{Suspension property}

\todo[inline]{Suspension}
\todo[inline]{Prove for standard}

\paragraph{Standard operations}
\todo[inline]{Standard operations}

\todo[inline]{Trivial proof for standard}
