...
\subsection{Normalisation for \Cattsua}
...
\[a  =_{\dim(a)} \SCoh {\insertion S P T} {A \sub {\kappa_{S,P,T}}} {\insertion L P M}\]
  ...
  \end{align*}
  By \cref{lem:pruned-bp,lem:insertion-irrel},
  \begin{equation*}
...
Using this lemma, we now show that the type theories \Cattsua and \(\Catt_{\sua'}\) are equivalent.
...
\begin{proof}
  ...
  We now assume that \(s\) is a unary composite \(\stdcoh {D^n} n \sub L\). Then, by \cref{lem:disc-insertion-1}:
  \[ t \equiv \SCoh T {\stdty {D^n} n \sub {\kappa_{D^n,P,T}}} M \]
  By appealing to inductive hypothesis on dimension and \cref{thm:std-insert-props}, \(\stdty {D^n} n \sub {\kappa_{D^n, P , T}} = \stdty T n\) and so \(t = \stdcoh T n \sub M \equiv L(\olsi P)\). Therefore the equality \(s = t\) is given by disc removal.

  Lastly, if \(s\) is not a unary composite or an identity, then we are done by \cref{lem:insertable} and the inductive hypothesise on dimension.
\end{proof}
...
Without loss of generality, we let \(a \equiv \SCoh S A L\), as variables do not reduce. We then split by cases on the reduction \(a \red b\), ignoring cases where both reductions are identical and ignoring cases which follow by symmetry of other cases.

\paragraph{Insertion:}

Let \(a \leadsto b\) be an insertion along redex \((S,P,T,\Gamma,L,M)\) such that \(a\) is not an identity or unary composite and \(\stdcoh T {\lh(P)}\) is a standard composite or identity. We now split on the reduction \(a \red c\).

\emph{Insertion on the inserted argument \(L(\olsi P)\):\quad} Suppose \(\stdcoh T {\lh(P)} \sub M\) admits an insertion along redex \((T, Q, U, \Gamma, M, N)\). Then:
...
\emph{Argument reduction on the inserted argument \(L(\olsi P)\):\quad}

\emph{Other reduction on the inserted argument \(L(\olsi P)\):\quad}

\emph{Reduction of non-inserted argument:\quad}
\todo[inline]{No other reductions}

\emph{Insertion:\quad}

\emph{Disc removal:\quad}

\emph{Endo-coherence removal:\quad}

\emph{Cell reduction:\quad}

\paragraph{Disc removal:}

\paragraph{Endo coherence removal:}

\todo[inline]{Possibly refer to cases from cattsu?}
