...
\[\begin{tikzcd}[column sep = small, cells={inner sep = 0}, arrows={no head}]
	&& {\color{red}\bullet} && \bullet &&&&&&&& \bullet \\
...
\begin{proof}
  ...
  \[\lfloor \mathcal{D} \sslash p \rfloor \vdash_{\mathcal{R}} \pi_p : \lfloor \mathcal{D} \rfloor \qquad \Gamma \vdash_{\mathcal{R}} \sigma \sslash p \]
  therefore, as \(\mathcal{O}\) supports pruning:
  \[\Gamma \vdash_{\mathcal{R}} \Coh {\lfloor \mathcal{D} \sslash p \rfloor} {(\arr s A t) \sub {\pi_p}} {\sigma \sslash p} : (\arr s A t) \sub {\pi_p} {\sigma \sslash p}\]
  and so by applying the conversion rule, it suffices to show that
  \[ \Gamma \vdash_{\mathcal{R}} B = (\arr s A t) \sub {\pi_p} {\sigma \sslash p}\]
...
\todo[inline]{This section introduces cattsu and cattsua}
\todo[inline]{cattsu from pruning, cattsua from insertion}
\todo[inline]{Heavily build on the previous sections}
\todo[inline]{Explore the theories more}
\todo[inline]{The main contribution will be to give normalisation}
\todo[inline]{Done by reduction}
\todo[inline]{Gives decidability of type checking}
\todo[inline]{Language can be implemented}
\todo[inline]{Introduce rust implementation}
\todo[inline]{Explain that this is nbe}
\todo[inline]{Section on nbe}
\todo[inline]{Build towards full nbe characterisation}
