...
\[
  ...
  \end{tikzcd}
\]
or may be written as \(f\colon x \to y\).\todo{should arrowheads be smaller?} Two cells are \emph{parallel} if they have the same source and target. Between any two parallel \(n\)-cells \(f\) and \(g\), we have a set of \((n+1)\)-cells between them. A \(2\)-cell \(\alpha : f \to g\) may be drawn as:
\[
  \begin{tikzcd}
    x & y
...
\todo[inline]{Two dimension/3 dimension composition}

In two dimensions we get our first deviation from
