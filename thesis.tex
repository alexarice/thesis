...
\begin{equation}
  ...
  \end{tikzcd}
\end{equation}
where the arrows labelled \(x\) and \(y\) send the unique point \(*\) to \(x\) and \(y\) respectively. Such a universal construction gives rise to two inclusions \(\inc_X : X \to X \vee Y\) and \(\inc_Y : Y \to X \vee Y\).
...
\begin{definition}
  ...
  \end{itemize}
  We lastly define a \emph{maximal} path of \(T\) to be a path \((p,0)\) such that \(T^p = \emp\). Such paths correspond to locally maximal variables of \(\lfloor T \rfloor\).
\end{definition}
...
As an example, take the tree \(T = [[\emp,\emp], \emp]\) from \cref{fig:tree-example}, and let \(\Gamma = (x : \star), (f : x \to x), (\alpha : f*f \to f)\). Then we can define the labelling \(L : \arr T \star \Gamma\) by:
...
\begin{definition}
  Let \(X\) be a member of \(\Ctx \uplus \Tree\), either some context \(\Gamma\) or some tree \(T\). We then define the \emph{structured syntax} classes \(\STerm_X\) of \emph{structured terms}, \(\SType_X\) of \emph{structured types}, and \emph{(\(\STerm\)-)labellings} \(\arr S A X\) for some tree \(S\) and structured type \(A\). These syntax classes for structured terms and types are generated by the following rules:
  \begin{mathpar}
    \inferrule{p : \Path_T}{\SPath(p) : \STerm_T}\and
    \inferrule{s : \STerm_{T_i}\\ 0 \leq i \leq n}{\Inc_i(s) : \STerm_{[T_0,\dots,T_n]}} \and
    \inferrule{S : \Tree\\ A : \SType_S \\ L : \arr S B X}{\SCoh S A L : \STerm_X}\and
    \inferrule{t : \Term_\Gamma}{\SOther(t) : \STerm_\Gamma} \\
    \inferrule{ }{\star : \SType_X}\and
    \inferrule{s : \STerm_X \\ A : \SType_X \\ t: \STerm_X}{\arr s A t : \SType_X}
  \end{mathpar}
  Labellings \(L : \arr S A X\) are defined as functions \(\Path_S \to \STerm_X\), similarly to term-labellings in \cref{sec:tree-contexts}. We note that the syntax for structured types is shared with the syntax for \Catt types, and will be careful to make it clear which syntax we are using.
\end{definition}
...
\begin{definition}
  Suppose \(X : \Ctx \uplus \Tree\). Define \(\lfloor X \rfloor\) to be \(\Gamma\) if \(X = \Gamma\) for some context \(\Gamma\) or \(\lfloor T \rfloor\) if \(X = T\) for some tree \(T\). Now, for a structured term \(s : \STerm_X\), a structured type \(A : \SType_X\), or a labelling \(L : \arr S B X\), we define:
  \[ \lfloor s \rfloor : \Term_{\lfloor X \rfloor} \qquad \lfloor A \rfloor : \Type_{\lfloor X \rfloor} \qquad \lfloor L \rfloor : \arr {\lfloor S \rfloor} {\lfloor B \rfloor} {\lfloor X \rfloor} \]
  by the equations:
  \begin{align*}
  ...
  \end{align*}
  and by defining \(\lfloor L \rfloor\) similarly to term labellings except \(\lfloor L \rfloor = \langle \lfloor A \rfloor, \lfloor L[0] \rfloor \rangle\) for labellings \(L : \arr {\emp} {A} {X}\) from the singleton tree.
\end{definition}
...
\begin{definition}
  Let \(L : \arr T B X\) be a labelling (with \(X : \Ctx \uplus \Tree\)). We define the application of \(L\) to a structured term \(s : \STerm_{T}\), a structured type \(A : \SType_T\), and a labelling \(M : \arr S A T\) to give:
  \[ s \sub L : \STerm_X \qquad A \sub L : \SType_X \qquad M \bullet L : \arr S {A \sub L} X\]
...
\begin{theorem}
  For any labelling \(L : \arr T B X\) and structured term \(s : \STerm_T\), structured type \(A : \SType_T\), or labelling \(M : \arr S A T\), we have:
...
\begin{proof}
  ...
  \item Suppose \(s\) is of the form \(\SPath(p)\). Then if \(\lfloor p \rfloor\) is not a \(0\)-dimensional variable, then an argument similar to the preceding case can be made. If instead \(\lfloor p \rfloor\) is of the form \([k]\) and \(T = [T_0,\dots,T_n]\) then first suppose that \(k < n + 1\) such that \(\lfloor [k] \rfloor \equiv \fst(\lfloor T_k \rfloor) \sub {\inc_{\lfloor T_k \rfloor}}\). Then:
    \begin{align*}
      ...
      &= \lfloor L[k] \rfloor
    \end{align*}
    where the last equality follows from the labelling \(L_k\) having type \(\arr {T_i} {\arr {\lfloor L[k] \rfloor} {B} {\lfloor L[k+1] \rfloor}} X\). The case where \(k = n+1\) is similar to above using \(\snd(T_n)\) instead of \(\fst(T_k)\) (as there is no tree \(T_k\) in this case).
  \end{itemize}
...
\todo[inline]{Main lemma (label application is sub application)}
\todo[inline]{Preliminary lemmas (assoc, unitality) follows from main theorem}

\todo[inline]{Recover wedge sums}
