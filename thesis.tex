...
\end{itemize}
\todo[inline]{de Brujin indices}
\todo[inline]{Fibred over natural instead of contexts}
\todo[inline]{Weakening operation}
\todo[inline]{Definition of identity substitution}
\todo[inline]{Introduce syntactic equality - inductive}
\todo[inline]{equality is defined inductively to avoid heterogeneous equality}
\todo[inline]{Signpost syntactic equality in formalisation}
\todo[inline]{Give basic properties about syntax, including associativity and unitality of substitutions}

\subsection{Typing and equality}
\label{sec:typing-equality}

\todo[inline]{One change is conversion rule}

\todo[inline]{Mention operations / pasting scheme}

\todo[inline]{Motivate the conversion rule, we want to be able to compose things that don't necessarily line up}

\todo[inline]{Introduce the set of rules R}

\todo[inline]{Give important rule for equality, transtive symmetric reflexive closure}

\todo[inline]{Initial example: Disc removal}

\todo[inline]{Example of lining up terms that are not definitionally equal}
\todo[inline]{Explain why we need the typing condition}
...
\vspace{7pt}
  \fbox{\begin{subfigure}{0.9852\textwidth}
    \begin{mathpar}
      \inferrule{\Gamma \vdash s : A \\ (\Gamma, s, t) \in \mathcal{R}}{\Gamma \vdash s = t} \and
...
\fbox{\begin{subfigure}{0.475\textwidth}
      \begin{align*}
        ...
        \Sigma (x) &= x\\
        \Sigma (\Coh \Delta A \sigma) &= \Coh {\Sigma(\Delta)} {\Sigma(A)} {\Sigma(\sigma)}\\
        \Sigma(\sigma) &= \unrestrict(\Sigma'(\sigma))\\[7pt]
        \Sigma'(\langle A \rangle) &= \langle \Sigma(A) \rangle\\
        \Sigma'(\langle \sigma, x \rangle) &= \langle \Sigma'(\sigma), \Sigma(t) \rangle\\
        \unrestrict\langle \arr s A t \rangle &= \langle A , s , t \rangle\\
...
\section{The set of operations \(\mathcal{O}\)}
\label{sec:support}
...
\paragraph{Supension property}

\todo[inline]{Supension}
...
\paragraph{Invertible generators}
...
\section{The set of equality rules \(\mathcal{R}\)}
\label{sec:ruleset}
