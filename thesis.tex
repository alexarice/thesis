...
We begin with an overview of the important concepts required for the rest of the thesis. Throughout, we will assume knowledge of various basic concepts from computer science, as well as a basic knowledge of category theory (including functor categories, presheaves, and (co)limits) and type theory. The following sections introduce a form of globular higher categories and give a definition of the type theory \Catt, close to the original definition.
...
We finally state the following well-known result (see \cite{finster2017type}) about isomorphisms between pasting contexts.
...
\begin{proposition}
  ...
  The insertion rule set, \insert, satisifies the suspension condition. It further satisfies the \(\mathcal{R}\)-substitution condition for any rule set \(\mathcal{R}\), and so also satisfies the weakening condition.
\end{proposition}
\begin{proof}
...
\begin{lemma}
  ...
  \[\insertion {(S \sslash P)} {P'} T = \insertion S P T \qquad \U \vdash_{\mathcal{R}} \pi_P \bullet \kappa_{S \sslash P,P',T} =^{\mathsf{max}} \kappa_{S,P,T} \]
  If we further have that \((S,P,T,\U,L,M)\) is an insertion redex then:
  \[\insertion {(\insertion {L} P {(\{\stdcoh T {\lh(P) - 1}\} \bullet M)})} {P'} {M} \equiv^{\mathsf{max}} \insertion L P M\]
\end{lemma}
\begin{proof}
...
The construction is therefore given by the colimit of the top-left border of the diagram. By a symmetric argument, it can be seen that performing the insertions in the opposite order also leads to a colimit of the same diagram. We finally state the lemma that formally states these ideas.
...
\begin{remark}
  The further conditions in each of \cref{lem:insertion-bd-1,lem:insertion-bd-2} could be dropped in favour of weakening the syntactic equalities to definitional equalitie in a theory with disc removal, as this would remove the distinction between standard terms and standard coherences. It was however more convenient to take this approach in the formalisation, and although the extra side conditions may seem arbitrary, the key result is that one of the above lemmas always holds.
\end{remark}
...
\begin{proof}
  ...
  \end{align*}
  Finally, the equality for standard terms follows from the equality for standard coherences, using \cref{thm:std-dr}.
\end{proof}
...
\end{lemma}
\begin{proof}
  See functions \func{Catt.Tree.Insertion.Properties}{insertion-tree-inserted-branch} and \func{Catt.Tree.Insertion.Properties}{label-from-inserted-branch} in module \module{Catt.Tree.Insertion.Properties}, and \func{Catt.Typing.Insertion.Equality}{κ-inserted-branch} in module \module{Catt.Typing.Insertion.Equality}.
\end{proof}
...
\subsection{Reduction for \Cattsua}
...
\subsection{Confluence of \Cattsua}
...
Finally, by \cref{lem:ins-comm-max}, we have that \(\kappa_{S,P,T} \circ (\insertion L P M) =_{\dim(S)} L\) and so \(b' = _{\dim(S)} c\) and since \(\dim(S) \leq \dim(a)\), we get \(b' =_{\dim(a)} c\) as required.
...
\todo[inline]{Cite rust implementation}
\todo[inline]{Explain purpose high level}
\todo[inline]{Uses an nbe style}
\todo[inline]{Evaluate terms that may not be closed}
\todo[inline]{Cite modal type theory paper as inspiration}
\todo[inline]{Quick overview of the 3 classes of syntax}
\todo[inline]{Diagram}
\todo[inline]{expression level syntax - represents the written text}
\todo[inline]{term level syntax - similarities to structured syntax}
\todo[inline]{In term level syntax, operations in catt are represented as constructors}
\todo[inline]{Normal form syntax - closest to catt syntax}
\todo[inline]{eval globally evaluates to normal form, primarily not via reduction}
\todo[inline]{Typechecking takes expression to term level syntax}
\todo[inline]{We assume all term level and normal form syntax is well-typed.}

\subsection{Syntax}
\label{sec:nbe-syntax}

\todo[inline]{Give all syntactic constructions}
\todo[inline]{Syntax for trees/tree labellings}
\todo[inline]{Explain span information}

\todo[inline]{Term level syntax}
\todo[inline]{Semantic interpretation?}

\todo[inline]{Normal form syntax}
\todo[inline]{Id and comp prioritised because of role in cattsua}
