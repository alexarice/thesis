...
We will see later (\cref{item:supp-tm-char-2}) that for well-typed terms \(t\) of typed \(A\) that the support of \(t\) is equal to \(\FV(t) \cup \FV(A)\) and that \(\Supp(A) = \FV(A)\) for well-formed types. Modifying \Catt to use support therefore does not change the theory.
...
\begin{lemma}
  ...
  For a Dyck word \(\mathcal{D} : \Dyck_d\), its associated context, type, and term are all well-typed:
  ...
  In addition to being a well-typed context, the context associated to a Dyck word is a ps-context; the following judgement holds:
...
\begin{definition}
  Let \(\mathcal{R}\) be an equality rule set. Define the reduction scheme \(\redr\) on well-typed terms, well-typed substitutions, and well-formed types to be generated by the rules in \cref{fig:reduction}. When it is clear which equality rule set is being used, we may simply write \(s \red t\) instead of \(s \redr t\).
\end{definition}
...
We highlight that our reduction scheme \(\redr\) is only defined between well-typed pieces of syntax. As this reduction will be used with rule sets \(\mathcal{R}\) which satisfy the preservation condition, there will be no additional burden of checking that typing is preserved while applying reductions. Therefore, we can prove that the reflexive symmetric transitive closure of reduction, \(\redrts\), is the same relation as equality on well-typed terms, given the similarity between the rules for reduction and the rules for equality.
...
\begin{proposition}
  \label{prop:red-sub}
  Suppose \(\mathcal{R}\) satisfies the substitution condition and let \(\sigma : \Delta \to \Gamma\) be a well-typed substitution. Then:
  \begin{align*}
  ...
  \end{align*}
  for well-typed terms \(s,t\), well-formed types \(A,B\), and well-typed substitutions \(\tau\) and \(\mu\). Furthermore, if \(\sigma \redr \tau\), then:
...
From this theorem it can be deduced that weakening, suspension, and applications of substitution are well-typed. Furthermore, equality in \Cattsu preserves the support of a term and preserves typing judgements. Such results are found in \cref{sec:ruleset}.

\todo[inline]{EH example}

\todo[inline]{Mention syllepsis will be done in implementation section}
...
\begin{proposition}
  The type theories generated from \su and \su' are equivalent. Terms are equal or well-typed in one theory exactly when they are equal or well-typed in the other, and similar properties hold for types and substitutions.
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  The only remaining case is where \(s \red u\) is an instance of pruning, which forces \(t \equiv \id(B,a)\) for some \(B\) and \(a\). By validity of \(s\), we must have \(n > 0\) and so \(A \equiv \arr b {A'} c\). Therefore:
  \begin{align*}
    ...
    &\equiv \id(A',b)
  \end{align*}
  Now by the validity of \(s\) we have \(\Gamma \vdash \{A,\id(B,a)\} : D^n\) and so by \cref{lem:disc-typing}, \(\Gamma \vdash \id(B,a) : A\) and hence by \cref{cor:id-typing} and uniqueness of typing:
...
We begin by defining the notion of canonical identity.
...
This allows us to now state and prove disc trivialisation.
