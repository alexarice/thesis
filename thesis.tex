...
\begin{definition}
  A \emph{tree} is inductively defined to be a (possibly empty) list of trees.
\end{definition}
...
\begin{proposition}
  ...
  If we suppose we have three contexts \(\Gamma\), \(\Delta\), and \(\Theta\) then there is a unique way of including each context into \(\Gamma \vee \Delta \vee \Theta\), that is there is a unique substitution \(\Gamma \to \Gamma \vee \Delta \vee \Theta\) which is built from a composite of inclusions and similarly for \(\Delta\) and \(\Theta\).
\end{proposition}
\begin{proof}
...
\begin{definition}
  Let \(T\) be a tree. \emph{Paths} \((p,n) : \Path_T\) are a pair of a list of natural numbers \(p\) and a natural number \(n\) such that \(p\) indexes a subtree of \(T^p\) of \(T\) and \(0 \leq n \leq \len(T^p) \).

  For path \((p, n) : \Path_T\), we obtain a variable of \(\lfloor T \rfloor\) by recursion on \(p\) as follows:
  \begin{itemize}
  \item Let \(T = [T_0,\dots,T_k]\). It is clear that \(\lfloor T \rfloor\) has exactly \(k+2\) variables of dimension \(0\), corresponding to (inclusion of) the first variable of each context \(\Sigma(\lfloor T_i \rfloor)\) as well as the variable corresponding to the inclusion of \(\snd(\Sigma(T_i))\). We then define \(\lfloor (\emp,n) \rfloor\) to be the \(n\)\textsuperscript{th} such variable, indexing from 0.
  \item Let \((k :: p, n)\) be a path, and \(T = [T_0,\dots,T_k,\dots]\). Then by recursion we have a variable \(\lfloor (p,n) \rfloor\) of \(\lfloor T_k \rfloor\). This gives a variable \(\Sigma(\lfloor (p,n) \rfloor)\) of \(\Sigma(\lfloor T_k \rfloor)\) which can be included into \(\lfloor T \rfloor\) by the appropriate inclusion to get \(\lfloor (k :: p,n) \rfloor\).
  \end{itemize}
