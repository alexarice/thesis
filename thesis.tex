...
\end{itemize}
These laws can be combined to create non-trivial emergent behaviour in a form not seen in the theory of \(1\)-categories. One critical example of this is known as the \emph{Eckmann-Hilton} argument, which states that the composition of two scalars, morphisms from the identity to the identity, commute. The argument proceeds by moving the two scalars around each other, as depicted in \cref{fig:eh}. This crucially uses both the interchange and unitality laws.
\newsavebox{\ehalpha}
...
\todo[inline]{For 3-categories and above this doesn't work}
\todo[inline]{Point to topological spaces}
\todo[inline]{Point to Eckmann Hilton}
\todo[inline]{Motivates the search for semistrict definitions}
\todo[inline]{Weak unit version}
\todo[inline]{Conjecture about dropping one}
\todo[inline]{Grey-categories}
\todo[inline]{Lack of definitions - let alone equiv}
\todo[inline]{Link to string diagrams}
\paragraph{Type theory and higher categories}
\todo[inline]{Move to type theory}
\todo[inline]{MLTT identity type}
\todo[inline]{Proof relevance}
\todo[inline]{Groupoid model}
\todo[inline]{Hott}
\todo[inline]{Hott contains a lot more}
\todo[inline]{infty topos}
\todo[inline]{Brunerei definition}
\todo[inline]{All groupoidal}
\todo[inline]{Definition of Catt}
\todo[inline]{Link to maltsiniotis/grothendieck}
\todo[inline]{Reference Thibaut}
\todo[inline]{Catt unusual, no computation}
\todo[inline]{We leverage this in current work}
\todo[inline]{Adding equality unifies operations, creating a semistrict theory}
\todo[inline]{Introduce cattsu cattsua, prove metatheoretic results}
\todo[inline]{Believe cattsua to be the first definition of a strictly unital and associative infinity category}
\paragraph{Overview}
