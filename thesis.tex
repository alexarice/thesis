...
\end{itemize}
The term \(N(t)\) is called the \emph{normal form} of \(t\). Such a algorithm allows equality of two term \(s\) and \(t\) to be decided by taking the normal form of each term and checking if they are syntactically equal. Normalisation can be extended to types and substitutions in a natural way.
...
Using a normalisation procedure, an algorithm can be given for type checking, which decides whether a given typing judgement holds. At key points in this type checking algorithm, two types need to be checked for equality, at which point normalisation can be used to decide the equality of the given types. We sketch such a type checking algorithm in \cref{sec:reduction}.

Such a type checking algorithm can be implemented, creating an interpreter for the language. This allows us to write larger terms, and have their validity automatically verified. In \cref{sec:towards-nbe}, we introduce our implementation of \Catt, \Cattsu, and \Cattsua, written in rust. This implementation supports features such as implicit arguments to terms, implicit suspension, and native support for trees and tree labellings. We will explain how the tool can be used, and use it to give larger examples of \Cattsua terms, including proofs of Eckmann-Hilton (see \cref{fig:eh}) and its higher dimensional coherence condition, the Syllepsis.

The implementation does not perform type checking using the algorithm sketched in \cref{sec:reduction}, instead taking an approach closer to normalisation by evaluation. \cref{sec:towards-nbe} explores this algorithm and presents some perspectives on applying normalisation by evaluation to semistrict versions of \Catt.

\todo[inline]{Describe further sections}
\todo[inline]{Include models section?}


\section{Reduction and type checking}
\label{sec:reduction}

\todo[inline]{Basics of reduction}

\todo[inline]{Give definitions of strong termination (local) confluence}

\todo[inline]{Decidability of equality}

\todo[inline]{Set up equality from rules}
