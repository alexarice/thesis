...
Given a globular set \(\mathcal{G}\), we will call the elements of \(G_n\) the \(n\)-cells and write \(f : x \to y\) for an \((n+1)\)-cell \(f\) where \(s_n(f) = x\) and \(t_n(f) = y\). Further we will drop the subscripts on \(s_n\) and \(t_n\) when they are clear. Further, we write \(s^n\) and \(t^n\) to denote \(s\) or \(t\) applied \(n\) times. We can now give the definition of a (strict) \(\infty\)-category.\todo{Add coinductive definition?}
...
We give a more graphical proof in \cref{fig:eh}. In this proof the \(\alpha\) is moved to the left of \(\beta\), though we equally could have moved it round the right, and the choice made was arbitrary.
