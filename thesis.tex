...
\begin{proof}
  ...
  \[\Gamma \vdash_{\mathcal{R}} \Coh {D^n} {U^n} {\{A,t\}} : B \implies \Supp(\{A,t\}) = \Supp(t) \]
...
\end{proposition}
\begin{proof}
  Take \((\Gamma, \Coh {D^n} {U^n} {\{A,t\}}, t) \in \dr\) and suppose \(\Gamma \vdash \Coh {D^n} {U^n} {\{A,t\}} : B\). Then by \cref{lem:ty-unique}:
  \[\Gamma \vdash B = U^n \sub {\{A,t\}} \equiv A\]
  By \cref{lem:disc-typing}, \(\Gamma \vdash t : A\) and so by the conversion rule \(\Gamma \vdash t : B\) as required.
\end{proof}
\todo[inline]{Check all discs for \(U^n\) instead of \(\wk(U^n)\)}
