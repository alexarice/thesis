...
We next define the free variables of each piece of syntax. These will be used to encode the condition of an operation being algebraic from the theory of non-invertible coherators. For a term \(t \in \Term_\Gamma\), a type \(A \in \Type_\Gamma\) and a substitution \(\sigma : \Delta \to \Gamma\) we define their free variables \(\FV(t)\), \(\FV(A)\), and \(\FV(\sigma)\) as subsets of the variables of \(\Gamma\) by mutual recursion.
...
Lastly we need to be able to describe pasting diagrams. As contexts model globular sets it is natural to treat pasting diagrams as a subset of contexts. We do this by introducing the judgements:

\[ \Delta \vdash_{\mathsf{ps}} \qquad \text{and}\qquad \Delta \vdash x : A \]
...
\todo[inline]{Reference Thibaut's paper}
