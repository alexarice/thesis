...
\begin{definition}
  A term-labelling \(L : \arr T A \Gamma\) from a tree \(T\) to context \(\Gamma\) with type \(A : \Type_\Gamma\) is a function \(\Path_T \to \Term_\Gamma\). If \(T = [T_0,\dots,T_n]\), then there are natural projections:
  \[ L_i : \arr {T_i} {\arr {L[i]} {A} {L[i+1]}} {\Gamma} \]
  given by \(L_i(p) = L(i :: p)\) for each \(i \in \{0,\dots,n\}\).
\end{definition}

For labellings to play the role of substitutions, a substitution \(\lfloor L \rfloor : \arr {\lfloor T \rfloor} A \Gamma\) will be defined for each term-labelling \(L : \arr T A \Gamma\). A natural way to define this substitution is by induction on the tree \(T\), which finally motivates the use of extended substitutions. Suppose we start with a labelling \(L : \arr {[T_0,\dots,T_n]} \star \Gamma\). To proceed, we will apply the inductive hypothesis to obtain a substitutions:
...
\begin{definition}
  Let \(L : \arr T A \Gamma\) be a term-labelling. We define the substitution:
  \[\lfloor L \rfloor : \arr {\lfloor T \rfloor} A \Gamma\]
  by induction on the tree \(T\) as \(\langle A, L[0] \rangle\) if \(T = \emp\) and:
...
\[ t_0\{L_0\}t_1\{L_1\}t_2\cdots t_n\{L_n\}t_{n+1} \]
...
Then we can define the labelling \(L : \arr T \star \Gamma\) by:
\[ L = x\bigl\{f*f\{\alpha\}f\{\id(f)\}f\bigr\}x\{f\}x \]
...
does not have nice computational properties. We look at two examples:
\begin{itemize}
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule{S : \Tree\\ A : \SType_S \\ L : \arr S B \U}{\SCoh S A L : \STerm_\U}\and
  ...
  \end{mathpar}
  Labellings \(L : \arr S A \U\) are defined as functions \(\Path_S \to \STerm_\U\), similarly to term-labellings in \cref{sec:tree-contexts}. We note that the syntax for structured types is shared with the syntax for \Catt types, and will be careful to make it clear which syntax we are using.
\end{definition}
...
\begin{definition}
  Suppose \(\U : \Ctx \uplus \Tree\). Define \(\lfloor \U \rfloor\) to be \(\Gamma\) if \(\U = \Gamma\) for some context \(\Gamma\) or \(\lfloor T \rfloor\) if \(\U = T\) for some tree \(T\). Now, for a structured term \(s : \STerm_\U\), a structured type \(A : \SType_\U\), or a labelling \(L : \arr S B \U\), we define:
  \[ \lfloor s \rfloor : \Term_{\lfloor \U \rfloor} \qquad \lfloor A \rfloor : \Type_{\lfloor \U \rfloor} \qquad \lfloor L \rfloor : \arr {\lfloor S \rfloor} {\lfloor B \rfloor} {\lfloor \U \rfloor} \]
  by the equations:
  \begin{align*}
  ...
  \end{align*}
  and by defining \(\lfloor L \rfloor\) similarly to term labellings except \(\lfloor L \rfloor = \langle \lfloor A \rfloor, \lfloor L[0] \rfloor \rangle\) for labellings \(L : \arr {\emp} {A} {\U}\) from the singleton tree. We refer to \(\lfloor a \rfloor\), \(\lfloor A \rfloor\) and \(\lfloor L \rfloor\) as the term, type, or substitution generated by \(a\),\(A\), or \(L\).
\end{definition}
...
\begin{definition}
  Let \(L : \arr T B \U\) be a labelling (with \(\U : \Ctx \uplus \Tree\)). We define the application of \(L\) to a structured term \(s : \STerm_{T}\), a structured type \(A : \SType_T\), and a labelling \(M : \arr S A T\) to give:
  \[ s \sub L : \STerm_\U \qquad A \sub L : \SType_\U \qquad M \bullet L : \arr S {A \sub L} \U\]
  ...
  \end{align*}
  and thus satisfy the computational properties given at the start of the section.
\end{definition}

The main theorem of this section is that the application of a labelling to a structured term is compatible with the map from structured syntax to \Catt syntax. We begin its proof with the following lemma:

\begin{theorem}
  \label{thm:structured-main}
  For any labelling \(L : \arr T B \U\) and structured term \(s : \STerm_T\), structured type \(A : \SType_T\), or labelling \(M : \arr S A T\), we have:
...
\begin{proof}
  ...
  \item Suppose \(s\) is of the form \(\SPath(p)\). Then if \(\lfloor p \rfloor\) is not a \(0\)-dimensional variable, then an argument similar to the preceding case can be made. If instead \(\lfloor p \rfloor\) is of the form \([k]\) and \(T = [T_0,\dots,T_n]\) then first suppose that \(k < n + 1\) such that \(\lfloor [k] \rfloor \equiv \fst(\lfloor T_k \rfloor) \sub {\inc_{\lfloor T_k \rfloor}}\). Then:
    \begin{align*}
      ...
      &= \lfloor L[k] \rfloor
    \end{align*}
    where the last equality follows from the labelling \(L_k\) having type \(\arr {T_i} {\arr {\lfloor L[k] \rfloor} {B} {\lfloor L[k+1] \rfloor}} \U\). The case where \(k = n+1\) is similar to above using \(\snd(T_n)\) instead of \(\fst(T_k)\) (as there is no tree \(T_k\) in this case).
  \end{itemize}

  The case for structured types follows by a simple induction using the case for terms. We now consider the case for a label \(M : \arr S A T\). Suppose \(S = [S_0,\dots,S_n]\). Then:
  \begin{align*}
...
\begin{definition}
  We define the suspension for a structured term \(a : \STerm_\U\), structured type \(A : STerm_\U\), and restricted substitution for a labelling \(L : \arr T A \U\), giving structured term \(\Sigma(a) : \STerm_\U\), structured type \(\Sigma(A) : \Sigma(A)\), and labelling \(\Sigma'(L) : \arr T {\Sigma(A)} {\Sigma(\U)}\). These are all defined by mutual induction as follows:
  \begin{align*}
    ...
    \Sigma'(L)(p) &= \Sigma(L(p))
  \end{align*}
  We further define an unrestriction operation that takes a labelling of the form \(M : \arr T {\arr s A t} \U\) and produces a labelling
  \[\unrestrict M  : \arr {\Sigma(T)} {A} \U \equiv s{M}t\]
...
\begin{definition}
  ...
  \[ \inc_S : \arr S \star {S \doubleplus T} \qquad \inc_T : \arr T \star {S \doubleplus T}\]
  by the equations:
  \begin{align*}
    \inc_S([k])&\equiv \SPath[k] & \inc_S(k :: p) &\equiv \SPath(k :: p)\\
    \inc_T([k])&\equiv \SPath[m + k] & \inc_T(k :: p) &\equiv \SPath (m + k :: p)
  \end{align*}
  and finally, we suppose \(L : \arr S A \U\) and \(M : \arr T A \U\) are labellings of the form:
  \[ L \equiv s_0\{L_0\}s_1\cdots s_n\{L_n\}t_0 \qquad M \equiv t_0\{M_0\}t_1\cdots t_n\{M_n\}t_{n+1} \]
  and define their concatenation to be the labelling:
  \[ s_0\{L_0\}s_1\cdots s_n\{L_n\}t_0\{M_0\}t_1\cdots t_n\{M_n\}t_{n+1} \]
  where \(L \doubleplus M : \arr {S \doubleplus T} A \U\).
\end{definition}
...
\[ \U \vdash L = M \iff \forall (p : \Path_T).\ \U \vdash L(p) = L(p)\]
...
\begin{definition}
  For a labelling \(L : \arr T A \U\), where \(\U : \Ctx \uplus \Tree\), we define the judgement:
  \[ \U \vdash L : T \]
  to mean that the labelling \(L\) is well-formed. This judgement is generated by the following rule:
  \begin{mathpar}
    % \inferrule{\U \vdash L[0] : \ty(L)}{\U \vdash L : \emp}\and
    % \inferrule{\forall k \leq n +1.\ \U \vdash L[k] : \ty(L)\\\forall i \leq n.\ \U\vdash L_i : T_i}{\U \vdash L : [T_0,\dots,T_n]}\and
  ...
  \end{mathpar}
  where \(\ty(L)\) is defined to be \(A\) for a labelling \(L : \arr T A \U\).
\end{definition}
...
\begin{proposition}
  Let \(L : \arr T A \U\), and suppose \(\U \vdash L : T\) and \(\U \vdash A\). Then \( \lfloor \U \rfloor \vdash \lfloor L \rfloor : \lfloor T \rfloor\).
\end{proposition}
\begin{proof}
  ...
  \[ \U \vdash L[0] : A \]
  By definition, this implies that \(\U \vdash L[0] : A\). Then, \(\lfloor L \rfloor \equiv \langle \lfloor A \rfloor, \lfloor L[0] \rfloor \rangle\), and so the following derivation can be obtained:
  \[
    \begin{prooftree}
      \infer0{\U \vdash A}
  ...
  From \(\U \vdash L : T\), we obtain \(\U \vdash L_i : T_i\) for each \(i \in \{0,\dots,n\}\), and so by inductive hypothesis we have \(\lfloor \U \rfloor \vdash \lfloor L_i \rfloor : \lfloor T_i \rfloor\). We have for each \(i\) that:
  \[\ty(\lfloor L_i \rfloor) \equiv \arr {\lfloor L[i] \rfloor} {\lfloor A \rfloor} {\lfloor L[i+1] \rfloor}\]
  and so as this is not the type \(\star\), the unrestriction \(\unrestrict \lfloor L_i \rfloor\) is well-formed. Furthermore, by construction of the unrestriction we have:
...
It can be shown that the reverse implication also holds: if \(\lfloor \U \rfloor \vdash \lfloor L \rfloor : \lfloor T \rfloor\) then \(\U \vdash L : T\). This is done in the formalisation module \module{Catt.Tree.Structured.Typing.Properties} by function \func{Catt.Tree.Structured.Typing.Properties}{Label-Ty}. By these results, many of the properties enjoyed by the typing judgements in \Cattr with a tame rule set \(\mathcal{R}\) also apply to the typing judgements for structured terms.
...
\begin{definition}
  For a path \(p : \Path_T\), a structured term \(s : \STerm_\U\), a structured type \(A : \SType_\U\), and a labelling \(L : \arr S B \U\), we define their supports \(\Supp(p)\), \(\Supp(s)\), \(\Supp(A)\), and \(\Supp(L)\) by mutual recursion:
  \begin{align*}
...
By using this notion of support, we are able to avoid a lot of ``boilerplate'' proof. The above definition of support more closely resembles the format of structured terms, and without this definition, most proofs concerning the support of a structured term would begin by simplifying a variable set similar to \(\Supp(\lfloor s \rfloor)\) to one of more similar to \(\Supp(\lfloor s \rfloor)\). Here, we instead give this equivalence proof once.

We end this section by giving alternative equality relations for labellings, which encapsulate the idea that a substitution is fully determined by where it sends locally maximal variables. These equalities are defined as follows for labellings \(L : \arr T A \U\) and \(M : \arr T B \U\):
\begin{align*}
...
\begin{theorem}
  Let \(L : \arr S A \U\) and \(M : \arr S B \U\) be labellings. Then the following rules are admissible:
  \begin{mathpar}
    \inferrule{\U \vdash L : S\\ \U \vdash M : S \\ L \equiv^{\max} M}{\U \vdash L = M}\and
    \inferrule{\U \vdash L : S\\ \U \vdash M : S \\ L \equiv^{\max} M}{\U \vdash A = B}
  \end{mathpar}
...
\begin{proof}
  ...
  First suppose that \(S = \emp\). Then the path \([0] : \Path_{\emp}\) is maximal and so \(\U \vdash L = M\) follows by the reflexivity of equality. The second rule follows from the uniqueness of typing, as we get \(\U \vdash L[0] : A\) and \(\U \vdash M[0] : B\) from the premises.

  Now suppose that \(S = [S_0,\dots,S_n]\). By inductive hypothesis, the following judgements hold for each \(i \in \{0,\dots,n\}\):
  \[ \U \vdash L_i = M_i \qquad \U \vdash \arr {L[i]} {A} {L[i+1]} = \arr {M[i]} {B} {M[i+1]}\]
  From the equalities on types, we immediately get that \(\U \vdash A = B\) as is required for the admissibility of the second rule, and also get that \(\U \vdash L[i] = M[i]\) for each \(0 \leq i \leq n+1\), which along with equality on (sub-)labellings above is sufficient to prove that:
  \[ \U \vdash L = M\]
...
\begin{definition}
  ...
  \end{align*}
  and then can define the \emph{source inclusion labelling} \(\incbd n + T : \arr {\bound n T} \star T\) and \emph{target inclusion labelling} \(\incbd n + T : \arr {\bound n T} \star T\) by:
...
\begin{definition}
  ...
  Finally, given a tree \(T\), we define the \emph{standard labelling} \(\stdlbl T n : D^n \to T\) by \(\stdlbl T n = \{\stdty T n, \stdcoh T n\}\).
\end{definition}
...
\begin{mathpar}
  ...
  \inferrule{\U \vdash A \\ \U \vdash t : A}{\U \vdash \{A,t\} : D^{\dim(A)}}\and
  \inferrule{T : \Tree \\ n \geq \dep(T)}{T \vdash \stdlbl T n : D^n}
\end{mathpar}
The admissibility of the above rules is routine to verify.

Using these construction, we can recover structured term definitions of the unary composite of a (structured) term \(t\) of type \(A\) of dimension \(n\) as \( \stdcoh {D^n} n \sub {\{A,t\}}\) and can define the identity of the same term \(t\) as \(\stdcoh {D^{n-1}} n \sub {\{A,t\}}\). Therefore, the rules for disc removal and endo-coherence removal can be rephrased in terms of structured syntax to get the following rules:
\begin{mathpar}
  \inferrule{\U : \Ctx \uplus \Tree\\ \U \vdash A \\ \U \vdash t : A \\ \dim(A) = n > 0}{ \U \vdash \stdcoh {D^n} n \sub {\{A,t\}} = t}\textsc{dr'}\\
  \inferrule{\U : \Ctx \uplus \Tree\\ T : \Tree \\ L : \arr S \star \U\\ n = \dim(A)\\\\ T \vdash A \\ T \vdash s : A \\ \Supp(s) = \Var(T) \\ \U \vdash L : T}{\U \vdash \SCoh T {\arr s A s} L = \stdcoh {D^n} n \sub { \{A, s\} \bullet L}}\textsc{ecr'}
\end{mathpar}
...
Lastly, under the presence of both disc removal and endo-coherence removal, the standard terms \(\stdtm T n\) for which \(n > \dep(T)\) can be shown to be equal to identities.

\begin{theorem}
  Suppose the equality rule set \(\mathcal{R}\) has both endo-coherence removal and disc removal. Let \(T\) be a tree and suppose \(n > \dep(T)\). Then:
  \[ T \vdash \stdtm T n = \stdcoh {D^n} {n+1} \sub {\{\stdty T n, \stdtm T n\}} \]
\end{theorem}
\todo[inline]{Proof of this}
\todo[inline]{remove standard label maybe}

Due to these two theorems, every standard term \(\stdtm T n\) with \(n \geq \dep(T)\) is equal to either the unique variable of the singleton context (when \(n = \dep(T) = 0\)), a standard composite (when \(n = \dep(T) > 0\)) or an identity (when \(n > \dep(T)\)), and hence completely classifying the well-formed standard terms.
...
We now consider the insertion of \(T_2\) into \(S\). Despite \(T_2\) having a lower depth than \(S\), it is still insertable, forming the following tree \(\insertion S P {T_2}\).
\[ S = \begin{tikzpicture}[yscale=0.7, every node/.style={scale=0.6},baseline=(x11.base)]
    \node [on grid] at (0,0) (x01) {$\bullet$};
    \node [on grid] at (-0.5,1) (x11) {$\bullet$};
    \node [on grid, blue] at (-0.9, 2)(x21) {$\bullet$};
